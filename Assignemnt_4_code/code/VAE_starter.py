...
def scatter_plot(latent_representations, labels):
    ...
    X_embedded = TSNE(n_components=2, learning_rate='auto', init = 'random').fit_transform(latent_representations)

    plt.figure(figsize=(10, 10))
    plt.scatter(X_embedded[:, 0], X_embedded[:, 1], cmap=discrete_cmap(10, 'jet'), c=labels, edgecolors='black')
    plt.colorbar()
    plt.grid()
    plt.show()
